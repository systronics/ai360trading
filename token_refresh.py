...
GH_REPO = os.environ.get("GITHUB_REPOSITORY", "")  # e.g. "username/ai360trading"
...
def verify_token_permissions(token: str) -> dict:
    """
    Check which permissions the new token has.
    Returns dict of permission: granted/declined.
    """
...
def get_repo_public_key(repo: str, gh_token: str) -> tuple:
    """Get GitHub repo's public key for secret encryption."""
...
def encrypt_secret(public_key_b64: str, secret_value: str) -> str:
    """Encrypt secret value using repo's public key (libsodium)."""
    try:
...
def update_github_secret(repo: str, gh_token: str, secret_name: str, secret_value: str) -> bool:
    """Update a GitHub Actions secret programmatically."""
    try:
...
def send_telegram_alert(message: str, success: bool = True) -> None:
    """Send Telegram notification about token refresh status."""
...
def main():
    ...
    try:
        ...
        # Step 3: Update GitHub Secret
        secret_updated = update_github_secret(
            GH_REPO, GH_TOKEN, "META_ACCESS_TOKEN", new_token
        )

        if not secret_updated:
            raise Exception("GitHub Secret update failed")

        # Step 4: Success alert
        perm_status = "✅ All permissions OK" if not missing_perms else f"⚠️ Missing: {', '.join(missing_perms)}"
        send_telegram_alert(
            f"META token refreshed successfully!\n\n"
            f"🔑 Secret: `META_ACCESS_TOKEN` updated in GitHub\n"
            f"🔐 Permissions: {perm_status}\n"
            f"⏳ New token valid for ~60 days\n"
            f"🔄 Next auto-refresh: in 50 days",
            success=True
        )
        ...
        send_telegram_alert(
            f"Token refresh FAILED!\n\n"
            f"Error: `{str(e)}`\n\n"
            f"⚠️ Action required: Manually refresh META_ACCESS_TOKEN in GitHub Secrets!\n"
            f"Go to: developers.facebook.com → Your App → Generate Token",
            success=False
        )
        sys.exit(1)
