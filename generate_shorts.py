...
def fetch_market_data():
    ...
    for name, (sym, curr) in tickers.items():
        try:
            ...
            val   = (f"{curr}{last:.2f}" if name == "usdinr"
...
# ─── SHORT 2 — TRADE SETUP / EDUCATION CARD (Madhur voice) ──────────────────
...
def share_to_facebook(short2_url, short3_url, market):
    ...
    msg += ("💡 Best time to learn and plan!\n🌍 India, US, UK, Brazil, UAE\n"
    ...
    for label, entity_id in targets:
        ...
        for attempt in range(1, FB_RETRY + 1):
            try:
                ...
                result = resp.json()
                if "id" in result:
                    print(f"  ✅ Facebook {label} posted — ID: {result['id']}")
                    success = True
                    break
                else:
                    ...
                    print(f"  ❌ Facebook {label} failed (attempt {attempt}/{FB_RETRY}) — Error {code}: {emsg}")
                    if code == 200:
                        print("  💡 FIX: Token needs 'publish_to_groups' permission scope.")
                        break
            except Exception as e:
...
async def main():
    ...
    try:
        await edge_tts.Communicate(s2_data.get("script", "Aaj ka lesson dekhte hain!"),
                                   "hi-IN-MadhurNeural", rate=rate_str).save(str(s2_audio_path))
    except Exception:
        await edge_tts.Communicate(s2_data.get("script", "Aaj ka lesson dekhte hain!"),
    ...
    try:
        await edge_tts.Communicate(s3_data.get("script", "Bhaiyo! Aaj ka update!"),
                                   "hi-IN-SwaraNeural", rate=rate_str).save(str(s3_audio_path))
    except Exception:
        await edge_tts.Communicate(s3_data.get("script", "Bhaiyo! Aaj ka update!"),
