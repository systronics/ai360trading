...
Uses: human_touch (hooks, TTS speed, SEO tags)
Mode-aware: market / weekend / holiday via CONTENT_MODE env var

Distribution:
  - YouTube Shorts only (upload via upload_youtube.py --type short)
  - Facebook Page text post with video URLs
  - Facebook Group: DEFERRED to Phase 3 (scope issue — see SYSTEM.md Section 4)
  - Instagram: DEFERRED to Phase 3 (see SYSTEM.md Section 4)

Target countries: India, USA, UK, Brazil, UAE, Canada, Australia
Last Updated: April 2026 — FB Group removed, Instagram deferred
"""
...
SW, SH    = 1080, 1920   # 9:16 portrait — NEVER change for Shorts
FPS       = 30
IST       = pytz.timezone("Asia/Kolkata")
now_ist   = datetime.now(IST)

MAX_SHORT_SECS = 58   # YouTube Shorts hard limit is 60s — use 58s for safety
...
def load_and_trim_audio(audio_path, label="Short"):
    """Load audio, trim to MAX_SHORT_SECS if over limit."""
    ...
    if voice_clip.duration < MIN_SHORT_SECS:
        print(f"⚠️ [{label}] Suspiciously short ({voice_clip.duration:.1f}s) — check TTS")
...
def fetch_market_data():
    print("  📡 Fetching market data for Shorts...")
    if CONTENT_MODE in ("holiday", "weekend"):
        print(f"  📅 {CONTENT_MODE.upper()} mode — using educational placeholders")
        return {
    ...
    for name, (sym, curr) in tickers.items():
        try:
            ...
            val   = (
                f"{curr}{last:.2f}"    if name == "usdinr"
                else f"{curr}{last:,.0f}" if name in ("btc", "sp500")
                else f"{curr}{last:,.2f}"
            )
...
def generate_short2_script(market):
    ...
    if CONTENT_MODE == "holiday":
        context = (f"Today is {HOLIDAY_NAME} — Indian market holiday. "
                   "Create educational investment lesson. Global: India, US, UK, Brazil, UAE.")
    elif CONTENT_MODE == "weekend":
        context = "Weekend — market closed. Educational investment lesson. Global audience."
    else:
        context = (
    ...
    if CONTENT_MODE in ("holiday", "weekend"):
        prompt = f"""Financial educator creating YouTube Shorts in Hinglish for ai360trading. Global audience.

{context}

HOOK (adapt naturally): {ht_hook}
...
{{"stock":"stock or index name","sentiment":"bullish or bearish or neutral","entry":"entry price","target":"target price","sl":"stop loss","horizon":"Intraday or Swing or Positional","insight":"one key Hinglish insight max 15 words","script":"40-second energetic Hinglish script. Hook. Setup. Subscribe CTA. Max 80 words."}}"""
    ...
    system_prompt = (
        "Expert Hinglish financial content creator for AI360Trading YouTube Shorts. "
        "Natural Hinglish — the way an experienced Indian trader speaks. "
        "Respond ONLY with valid JSON."
    )

    print("  🤖 Generating Short 2 script (Madhur voice) via ai_client...")
    try:
        data = ai.generate_json(prompt=prompt, system_prompt=system_prompt, content_mode=CONTENT_MODE)
        if not data:
            raise ValueError("Empty response")
        print(f"  ✅ Short 2: {data.get('stock')} | via {ai.active_provider}")
        return data
    except Exception as e:
        print(f"  ⚠️ Short 2 error: {e} — using fallback")
        return {
            "stock":    "FINANCIAL EDUCATION",
            "sentiment": "positive",
            "entry":    "Learn Investing",
            "target":   "Build Wealth",
            "sl":       "Start Today",
            "horizon":  "Long Term",
...
def make_short2_frame(script_data, market):
    ...
    header_label = {
        "holiday": "HOLIDAY LEARNING", "weekend": "WEEKEND EDUCATION"
    }.get(CONTENT_MODE, "TRADE SETUP")
    ...
    stock = script_data.get(
        "stock",
        "FINANCIAL EDUCATION" if CONTENT_MODE in ("holiday", "weekend") else "NIFTY 50"
    )
    ...
    cta = (
        ...
        if CONTENT_MODE in ("holiday", "weekend")
        else "📲 SUBSCRIBE FOR DAILY SIGNALS"
    )
...
def generate_short3_script(market):
    ...
    if CONTENT_MODE == "holiday":
        context = (f"Today is {HOLIDAY_NAME} — Indian market holiday. "
                   "Educational overview. What should investors do on holidays? Global: India, US, UK, Brazil, UAE.")
    elif CONTENT_MODE == "weekend":
...
HOOK (adapt naturally): {ht_hook}
Respond ONLY with valid JSON:
{{"mood_summary":"inspiring Hinglish message max 8 words","key_level":"one investment action or topic max 5 words","script":"40-second inspiring Hinglish script. Hook. What to learn. Subscribe CTA. Max 80 words."}}"""
    else:
        prompt = f"""Energetic Indian market commentator for ai360trading YouTube Shorts.
{context}
HOOK (adapt naturally): {ht_hook}
Respond ONLY with valid JSON:
{{"mood_summary":"overall market mood Hinglish max 10 words","key_level":"most important Nifty level today","script":"40-second energetic Hinglish script. Hook. Nifty global cues. Subscribe CTA. Max 80 words."}}"""

    system_prompt = (
        "Energetic Hinglish market commentator for AI360Trading. "
        "Natural, punchy, relatable. Respond ONLY with valid JSON."
    )
    ...
    try:
        ...
        print(f"  ⚠️ Short 3 error: {e} — using fallback")
        return {
...
def make_short3_frame(script_data, market):
    ...
    accent     = GOLD if CONTENT_MODE in ("holiday", "weekend") else (
                 BULL_GREEN if overall_up else BEAR_RED)
    ...
    header = {
        "holiday": "HOLIDAY SPECIAL", "weekend": "WEEKEND WISDOM"
    }.get(CONTENT_MODE, "MARKET PULSE")
    ...
    if CONTENT_MODE in ("holiday", "weekend"):
        ...
        draw_text_outlined(draw, script_data.get("mood_summary", "LEARN"),
                           SW // 2, 430, get_font(FONT_BOLD_PATHS, 80), WHITE, outline=2)
        draw_text_outlined(draw, "& GROW",
                           SW // 2, 520, get_font(FONT_BOLD_PATHS, 65), accent, outline=2)
    else:
        draw.text((SW // 2, 315), "NIFTY 50",
                  font=get_font(FONT_BOLD_PATHS, 48), fill=(180, 205, 240), anchor="mm")
        draw_text_outlined(draw, market["nifty"]["val"],
                           SW // 2, 430, get_font(FONT_BOLD_PATHS, 110), WHITE, outline=2)
        draw_text_outlined(draw, market["nifty"]["chg"],
                           SW // 2, 520, get_font(FONT_BOLD_PATHS, 65), accent, outline=2)
    ...
    cta = (
        "🔔 SUBSCRIBE FOR DAILY LEARNING"
        if CONTENT_MODE in ("holiday", "weekend")
        else "🔔 SUBSCRIBE FOR DAILY UPDATES"
    )
...
def upload_short(youtube, video_path, title, description, tags):
    ...
    body = {
        "snippet": {
            ...
            "tags":        tags[:30],
            "categoryId":  "22"   # People & Blogs — better for Shorts algorithm
        },
        "status": {
...
# ─── FACEBOOK PAGE (Group posting deferred to Phase 3) ───────────────────────

def post_to_facebook_page(short2_url, short3_url, market):
    """
    Post short links to Facebook PAGE only.
    Facebook GROUP posting is DEFERRED to Phase 3 — requires publish_to_groups
    token scope + Meta App approval. See SYSTEM.md Section 4 for re-enable steps.
    """
    token   = os.environ.get("META_ACCESS_TOKEN", "")
    page_id = os.environ.get("FACEBOOK_PAGE_ID", "")

    if not token or not page_id:
        print("  ⚠️ Facebook credentials missing — skipping Page post")
        return
    ...
    msg += (
        "💡 Best time to learn and plan!\n"
        "🌍 India, US, UK, Brazil, UAE\n"
        "🌐 https://ai360trading.in\n"
        "📱 https://t.me/ai360trading\n\n"
        "#ai360trading #StockMarket #Investing #Shorts"
    )
    ...
    for attempt in range(1, FB_RETRY + 1):
        try:
            ...
            result = resp.json()
            if "id" in result:
                print(f"  ✅ Facebook Page posted — ID: {result['id']}")
                return
            else:
                error = result.get("error", {})
                code  = error.get("code", "?")
                emsg  = error.get("message", str(result))
                print(f"  ❌ Facebook Page failed (attempt {attempt}/{FB_RETRY}) — Error {code}: {emsg}")
        except Exception as e:
            print(f"  ⚠️ Facebook Page error (attempt {attempt}/{FB_RETRY}): {e}")
        if attempt < FB_RETRY:
...
async def main():
    ...
    base_tags = (
        ["ai360trading", "Nifty", "StockMarket", "Investing", "Finance",
         "GlobalInvesting", "USStocks", "UKInvesting", "BrazilMarket",
         "IndiaInvesting", "UAEInvesting", "FinancialLiteracy", "Hinglish"]
        + (["Nifty50", "TradingIndia", "TradeSetup", "TechnicalAnalysis"]  if CONTENT_MODE == "market"
           else ["WeekendLearning", "PersonalFinance", "WealthBuilding"]   if CONTENT_MODE == "weekend"
           else ["HolidayLearning", "MarketHoliday", "InvestmentTips"])
    )
    tags = list(dict.fromkeys(base_tags + ht_tags))

    # ✅ TTS speed from human_touch
    ...
    try:
        await edge_tts.Communicate(
            ...
            "hi-IN-MadhurNeural", rate=rate_str
    ...
    try:
        await edge_tts.Communicate(
            ...
            "hi-IN-SwaraNeural", rate=rate_str
    ...
    # ── Facebook Page post (Group DEFERRED to Phase 3) ────────────────────────
    post_to_facebook_page(short2_url, short3_url, market)
    ...
    print(f"  AI Used  : {ai.active_provider}")
    print(f"  FB Group : DEFERRED to Phase 3")
    print(f"  Instagram: DEFERRED to Phase 3")
    print(f"{'=' * 50}\n")
