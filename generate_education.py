...
MIN_SLIDES   = 22          # ✅ FIXED: enforce 22 slides minimum
WORDS_TARGET = "80-100"    # ✅ FIXED: was 50-70
...
# ─── SCRIPT GENERATION via ai_client (full fallback chain) ───────────────────
def generate_edu_slides(topic, part1_url):
    """
    Uses ai_client → Groq → Gemini → Claude → OpenAI → templates.
    Never calls Groq directly. Always 22 slides, 80-100 words each.
    """
    today = datetime.now().strftime("%A, %d %B %Y")

    # Build 22-slide structure from topic; expand if topic has fewer headings
    topic_slides = topic.get("slides", [])
    slide_headings = [s.get("heading", f"Slide {i+1}") for i, s in enumerate(topic_slides)]

    # Pad to 22 slides with intelligent expansion headings
    EXPANSION_SLIDES = [
    ...
    # Get rotating hook from human_touch for slide 1
...
IMPORTANT RULES:
- Each slide content MUST be {WORDS_TARGET} words of spoken Hinglish (NOT fewer — this is required for 10+ minute video)
- Use natural conversational Hinglish — Hindi + English mix
- Include real Indian stock/market examples where possible
- Slide 17 must include USA/UK market context for global CPM
- Slide 22 must end with: "Subscribe karo, Telegram join karo t.me/ai360trading, aur Part 1 dekhna mat bhoolna!"
- If topic is holiday/weekend mode, keep content educational, not live market specific
...
{{
  ...
  "slides": [
    {{
      ...
      "content": "spoken Hinglish content {WORDS_TARGET} words — clear, simple, examples",
      "key_takeaway": "one line summary of this slide"
    }}
  ]
}}"""

    print(f"🤖 Generating education script via ai_client: {topic['title']}...")

    # ✅ Use ai_client — full Groq→Gemini→Claude→OpenAI→template fallback
    data = ai.generate_json(prompt, content_mode=CONTENT_MODE, lang="hi")

    if data and data.get("slides") and len(data["slides"]) >= 10:
        slides = data["slides"]
        # Ensure we have exactly MIN_SLIDES — pad if AI returned fewer
        while len(slides) < MIN_SLIDES:
            slides.append({
                "title": slide_headings[len(slides)] if len(slides) < len(slide_headings) else "Key Lesson",
                "content": "Trading mein sabse important cheez hai discipline aur patience. Jo trader apni emotions control karta hai, woh long term mein zaroor profitable hota hai. Yeh aapke liye ek important reminder hai.",
                "key_takeaway": "Discipline aur patience — trading ka asli secret"
            })
        print(f"✅ {len(slides)} education slides generated")
        data["slides"] = slides[:MIN_SLIDES]  # cap at 22
        return data
    else:
        print("⚠️ AI returned insufficient slides — building from topic calendar directly")
        fallback_slides = []
        for i, heading in enumerate(slide_headings):
            if i < len(topic_slides):
                ...
                if not content:
                    content = f"{heading} ke baare mein aaj hum detail mein samjhenge. Yeh concept Indian stock market mein bahut important role play karta hai."
            else:
                content = f"{heading} trading ka ek bahut important aspect hai. Sabse pehle aapko yeh samajhna hoga ki successful traders kaise sochte hain aur apne decisions kaise lete hain. Practice aur consistency se yeh skill develop hoti hai."
...
async def gen_voice(text, path):
    # ✅ Use human_touch TTS speed variation — anti-AI-penalty
...
def check_duration(video_path):
    """Fail hard if video is under 8 minutes — mid-roll ads need 8+ min."""
    try:
        ...
        if dur < 480:
            print(f"❌ CRITICAL: Video is {dur:.0f}s — under 8 minutes! Mid-roll ads will NOT activate.")
            print("❌ Fix: Check slide count (need 22) and word count per slide (need 80-100 words).")
            sys.exit(1)
        elif dur < 600:
            print(f"⚠️ WARNING: Video is {dur:.0f}s ({dur/60:.1f} min) — close to 8min threshold. Target is 10-12 min.")
        else:
...
async def run():
    ...
    # 3. Generate 22-slide script via ai_client (full fallback chain)
    data   = generate_edu_slides(topic, part1_url)
    slides = data["slides"]
    print(f"📊 Total slides: {len(slides)} (target: {MIN_SLIDES})")
    ...
    # ✅ Humanize title and description via human_touch
    vid_title = ht.humanize(vid_title, lang="hi")[:100]
    ...
    extra_tags = [topic["title"], topic["category"], "Trading Education", "ai360trading",
                  "Stock Market India", "Learn Trading", "NSE", "BSE", "Hinglish", topic["level"]]
    all_tags = list(dict.fromkeys(seo_tags + extra_tags))  # deduplicate
    ...
    for i, s in enumerate(slides):
        ...
        duration   = voice_clip.duration + 1.2   # ✅ FIXED: was 0.8, now 1.2s pause
        ...
        clips.append(clip)
        print(f"  Slide {i+1}/{len(slides)} ready ({voice_clip.duration:.1f}s voice + 1.2s pause)")
    ...
    # 8. Save Part 2 ID and update Part 1 description
    if part2_id:
