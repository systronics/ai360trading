...
def run_trading_cycle():
    ...
    if is_market_hours(now):
        ...
        for i, r in enumerate(trade_zone):
            ...
            # Clear old _EX flag so exit logic works correctly on re-entry
            # Without this: same stock traded twice → old _EX flag blocks new exit
            ex_flag_key = f"{key}_EX"
            mem = ','.join(p for p in mem.split(',') if p.strip() and p.strip() != ex_flag_key)
            print(f"[ENTRY] {sym}: cleared old _EX flag for fresh monitoring")
        ...
        for sheet_row, r in traded_rows:
            ...
            # ── Exit Logic ─────────────────────────────────────────────────
            # Include entry date in ex_flag — prevents old _EX flag from
            # blocking exit of a re-entry on the same stock
            # e.g. NATIONALUM exited Mar 5, re-entered Mar 12
            # Old flag: NSE_NATIONALUM_EX (blocks new trade)
            # New flag: NSE_NATIONALUM_EX_20260312 (fresh per entry)
