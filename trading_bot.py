...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
v14.0 CHANGES vs v13.5 — 7 CRITICAL FIXES + 3 NEW FEATURES

CRITICAL BUG FIXES:
1. TELEGRAM CHANNEL SWAP FIX [CRITICAL]
   Previous: CHAT_ADVANCE = os.environ.get('CHAT_ID_PREMIUM')  ← WRONG
             CHAT_PREMIUM = os.environ.get('CHAT_ID_ADVANCE')  ← WRONG
   Fix:      CHAT_ADVANCE = os.environ.get('CHAT_ID_ADVANCE')  ← CORRECT
             CHAT_PREMIUM = os.environ.get('CHAT_ID_PREMIUM')  ← CORRECT
   Impact: Advance and Premium channels were receiving each other's messages.

2. ADVANCE vs PREMIUM DIFFERENTIATION [CRITICAL]
   Previous: Good Morning sends IDENTICAL full_gm to both channels.
   Fix: Advance = full trade details (entry, SL, target, P/L)
        Premium = Advance + CE options candidate flag per trade
        Basic   = market mood + active count only (no entry data)

3. BOTMEMORY SHEET READ [CRITICAL]
   Previous: Python read _CAP/_MODE/_SEC/_RANK from T4 text string.
   AppScript v14.1+ writes these to BotMemory SHEET, not T4.
   Fix: Python now reads BotMemory SHEET for _CAP/_MODE/_SEC/_RANK.
   T4 cell is kept for Python-only state: TSL, MAX, LP, ATR, EXDT, AM flags.

4. RESULT / EVENT DAY SKIP [NEW]
   If stock gaps >6% at open (vs previous close estimate) → skip entry.
   SBIN loss happened on result day — this prevents repeat.

5. INDIAN HOLIDAY CHECK [NEW]
   trading_bot.py now checks NSE holidays independently.
   Previously relied entirely on GitHub Actions cron schedule.

6. MAX_TRADES SYNC [FIX]
   Previous: MAX_TRADES hardcoded as 5 (bot), AppScript had 8.
   Fix: MAX_TRADES = 8 in Python, matching AppScript v14.1+.

7. CAPITAL FALLBACK FIX [FIX]
   Previous: When BotMemory read failed, defaulted to CAPITAL_PER_TRADE=10000.
   Fix: Proper 3-tier fallback (7000/10000/13000) from BotMemory sheet.
   CAPITAL_PER_TRADE constant removed — capital always comes from BotMemory.

NEW FEATURES:
8. MID-DAY PULSE 12:28–12:38 [RESTORED]
   Sends P/L snapshot of all open trades to Advance + Premium.
   Basic gets a simple "market is moving" message.

9. MARKET CLOSE SUMMARY 15:15–15:45 [RESTORED]
   Full EOD summary: wins/losses, open overnight, P/L.
   Sent to Advance + Premium in detail, Basic gets condensed version.

10. CE FLAG GATED BY RANK [NEW]
    CE candidate flag shown only if BotMemory _RANK ≤ 5 (sector leaders).
    Prevents options alerts on low-conviction stocks.

UNCHANGED FROM v13.5:
  All TSL calculation logic (VCP/MOM/STD params)
  Memory helpers (T4 string helpers for TSL/MAX/LP/ATR/EXDT)
  get_sheets(), get_market_regime(), price_sanity(), calc_hold_*()
  options_hint(), _read_atr_from_nifty200()
  All exit logic (hard loss, TSL hit, target hit, min hold rules)
